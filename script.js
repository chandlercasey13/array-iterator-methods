...
let veryOldInventors = inventors.filter((people) => {
  return people.year < 1600 && people.year > 1499;
});
...
// Enter your solution code here:
let inventorNames = inventors.map((people) => {
...
let firstLast = people.map((person) => {
  ...
  let reversedSplit = splitPerson.reverse()
  let joinedReverse = reversedSplit.join()
...
// Enter your solution code here:
let commentById = comments.find((person)=>{
    if(person.id === 823423)
    return person.text
})
...
let idx = comments.findIndex((person)=>{
    if (person.id===123523){
        return person
    }
})
